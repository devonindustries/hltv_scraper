...
def build_link(category, date_range, _map):
    ...
    return 'https://www.hltv.org/stats/' + category + '?' + date_range + _map
...
def scrape_match_data(driver, weblink):
    ...
    matches = pd.DataFrame({'Date' : dates,
                        ...
                        'Team2' : team2,
                        'Map' : _map,
                        'Team1 Score' : t_1_score,
                        'Team2 Score' : t_2_score})

    return matches

# Below is the one function to rule them all.
def scrape(driver, category, date_range, _map):
    ...
    weblink = build_link(category, date_range, _map)
